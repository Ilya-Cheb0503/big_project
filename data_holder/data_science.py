...
async def data_inf(update, context, admin_id):
    ...
    categories_pref = 'Статистика по категориям, которые наиболее интересны пользователям:\n'
    categories_stat = 'На текущий момент нет категорий, которые наиболее интересных пользователям.\n\n'

    replies_pref = 'Статистика по вакансиям, на которые пользователи откликаются чаще всего:\n'
    ...
    with open(full_path, 'rb') as txt_file:
        await context.bot.send_document(chat_id=admin_id, document=txt_file, filename='Метрика.txt')
